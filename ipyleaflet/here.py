...
def build_tiles_url(**kwargs):
    ...
    >>> build_tiles_url(appkey="foobar")
    'https://3.base.maps.ls.hereapi.com/maptile/2.1/maptile/newest/normal.day/{z}/{x}/{y}/256/png8?lg=eng&appkey=foobar'
    >>>
    ...
    raise ValueError(
        "HERE APIKEY or APP_CODE/APP_ID needed to be in params or env. vars.")


def basemap_to_tiles(**kwargs):
    """Return a dict with tile layer information for HERE map tiles.
    
    Example:
    
    >>> basemap_to_tiles(app_id="foo", app_code="bar")
    {'url': 'https://4.traffic.maps.api.here.com/maptile/2.1/traffictile/newest/normal.day/{z}/{x}/{y}/256/png8?lg=eng&app_id=foo&app_code=bar',
     'min_zoom': 1,
     'max_zoom': 18,
    ...
    """
    params = dict(
        url = build_tiles_url(**kwargs),
        min_zoom = kwargs.get("min_zoom", 1),
        max_zoom = kwargs.get("max_zoom", 18),
        attribution = '&copy; <a href="https://here.com">HERE.com</a>',
        name = "HERE",
    )
    return params  # better use TileLayer?
